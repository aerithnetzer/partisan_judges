...
def get_embeddings(text, model_name_or_path):
    ...
    corpus_embeddings = model.encode(
        text, batch_size=1024, show_progress_bar=True, convert_to_tensor=True
    )
...
def length_adjustment_bin(df, length_column="length", minimum_length=10):
    ...
    df[f"{length_column}_bin"] = pd.cut(
        df[length_column],
        bins=bins,
    )
    df["evidence_mean"] = df.groupby(f"{length_column}_bin")[
        "avg_evidence_score"
    ].transform("mean")
    df["evidence_adj"] = df["avg_evidence_score"] - df["evidence_mean"]

    df["intuition_mean"] = df.groupby(f"{length_column}_bin")[
        "avg_intuition_score"
    ].transform("mean")
...
def evidence_minus_intuition_score(
    df, evidence_column="evidence_adj", intuition_column="intuition_adj"
):
    df[["evidence_z", "intuition_z"]] = df[[evidence_column, intuition_column]].apply(
        zscore
    )
...
def preprocess(df, args):
    ...
    try:
        print(
            "sample top100 ", df[(df.fraction_top100 < 0.05)].sample(10).text.tolist()
        )
...
def main(args):
    ...
    for batch in tqdm(list_df):
        ...
        intuition_embeddings = get_embeddings(
            intuition_keywords, args.model_name_or_path
        )
        intuition_embeddings = torch.mean(intuition_embeddings, dim=0)

        evidence_sim = torch.cat(
            (evidence_sim, util.cos_sim(text_embeddings, evidence_embeddings).cpu()), 0
        )
        intuition_sim = torch.cat(
            (intuition_sim, util.cos_sim(text_embeddings, intuition_embeddings).cpu()),
            0,
        )
    ...
    df = length_adjustment_bin(
        df, length_column=length_column, minimum_length=args.length_threshold
    )
    df = evidence_minus_intuition_score(
        df, evidence_column="evidence_adj", intuition_column="intuition_adj"
    )
...
def main_temporal(args):
    ...
    for decade, group in tqdm(grouped_df):
        ...
        text_embeddings = get_embeddings(
            decade_text, f"{args.temporal_models_path}/decade-{decade}-model"
        )
        evidence_embeddings = get_embeddings(
            evidence_keywords, f"{args.temporal_models_path}/decade-{decade}-model"
        )
        intuition_embeddings = get_embeddings(
            intuition_keywords, f"{args.temporal_models_path}/decade-{decade}-model"
        )
    ...
    combined_df = length_adjustment_bin(
        combined_df, length_column="chunk_length", minimum_length=10
    )
    combined_df = evidence_minus_intuition_score(
        combined_df, evidence_column="evidence_adj", intuition_column="intuition_adj"
    )
